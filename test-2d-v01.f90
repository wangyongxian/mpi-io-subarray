! 利用MPI IO对分布式二维数组进行并行IO
! Wang Yong-Xian
! 2019-04-07
!
! compile:  mpif90 test-2d-v01.f90 -o test-2d-v01.exe
! run:      yhrun -n 8 -N 2 -p TH_NET1 ./test-2d-v01.exe
!
program test
    use mpi
    implicit none

    integer, parameter :: m = 1024, n = 512
    integer, parameter :: npi = 2, npj = 4, np = npi * npj
    
    real(kind=4), allocatable :: dat(:, :), cpy(:, :)
    integer            :: local_array_size
    integer            :: nprocs, rank
    integer            :: i, j, count, ierr
    ...
    do i = 0, lsizes(1)-1
        do j = 0, lsizes(2)-1
            if ( dat(i,j) /= cpy(i,j) ) then
                write(*, *) "rank / pos(i,j) / dat / cpy: ", rank, i, j, dat(i,j), cpy(i,j)
            end if
        end do
    end do
...
subroutine initialize() 
    ...
    allocate( dat(0:lsizes(1)-1, 0:lsizes(2)-1) )
    allocate( cpy(0:lsizes(1)-1, 0:lsizes(2)-1) )

    do i = 0, lsizes(1)-1
        do j = 0, lsizes(2)-1
            dat(i, j) = i * lsizes(2) + j + 1 + rank * 1000
            cpy(i, j) = 0
        end do
    end do
end subroutine
